...
/// <summary>Loads a collection folder the same way the app does, so both see the same requests.</summary>
internal static class CollectionLoader
{
    public static LoadedCollection Load(string folder)
    {
        ...
        // The generated set joined with the user's overlay, in memory, exactly as the app does it.
        ...
        if (Directory.Exists(requestsFolder))
        {
            ...
            {
                requests.Add(serializer.DeserializeRequest(File.ReadAllText(file)));
            }
        }
...
internal sealed record LoadedCollection(CollectionDefinition Definition, IReadOnlyList<RequestDefinition> Requests);
